...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        if config.hyperparameters['spgd_factor']>0:
            self.spgd_optim = SGD_CPS(lr=config.hyperparameters['spgd_lr'], momentum=config.hyperparameters['spgd_momentum'])
        else:
    ...
    def run_n_episodes(self, num_episodes=None, show_whether_achieved_goal=True, save_and_print_results=True):
        ...
        while self.episode_number < num_episodes:
            ...
            if self.config.save_model and self.config.save_per_episode is not None:
                if self.episode_number % self.config.save_per_episode:
    ...
    def pick_spgd_action(self):
        spgd_factor = self.hyperparameters['spgd_factor']
        spgd_begin = self.hyperparameters['spgd_begin']
        spgd_warmup = self.hyperparameters['spgd_warmup']
        n_step=self.episode_step_number_val
        if spgd_factor<=0 or n_step < spgd_begin:
            return 0,0
        lbd = spgd_factor
        if n_step<spgd_warmup:
            lbd = (n_step-spgd_begin)*spgd_factor/(spgd_warmup - spgd_begin)
        grad = self.info['grad']
        spgd_act = self.spgd_optim.step(grad)
        return lbd, spgd_act


    def conduct_action(self, action):
        """Conducts an action in the environment"""
        if self.hyperparameters['spgd_factor']>0:
            lbd, spgd_action = self.pick_spgd_action()
            action = (1-lbd)*action + lbd*spgd_action
    ...
    def pick_action(self, state=None):
        """Takes a step in the game. This method must be overriden by any agent"""
        raise ValueError("pick_action needs to be implemented by the agent")

    def pick_best_action(self, state=None):
        """Takes a step in the game. This method must be overriden by any agent"""
        raise ValueError("pick_best_action needs to be implemented by the agent")

    def evaluate(self, reward_threshold=None,max_steps=None,save_name=None):
        print('*********  Begin Evaluating *********')
        self.reset_game()
        done = self.done
        self.episode_step_number_val = 0
        results={'power':[],'pulse':[],'reward':[]}
        while not done:
            self.episode_step_number_val += 1
            if self.config.test_render:
                self.environment.render()
            self.action = self.pick_best_action()
            self.conduct_action(self.action)
            self.state = self.next_state
            info = self.info
            done = self.done
            results['power'].append(info['power'])
            results['pulse'].append(info['pulse'])
            results['reward'].append(info['reward'])
            print(f"Step={self.episode_step_number_val}, Reward={self.reward}, Power={info['power']} ({self.environment.orig_f2_power},{self.environment.max_f2_power})")
            if reward_threshold is not None and self.episode_step_number_val >10:
                done = done or np.mean(results['reward'][-10:])>=reward_threshold
            if max_steps is not None:
                done = done or self.episode_step_number_val >= max_steps
        if save_name is None:
            save_name = 'evaluation_results'
        save_path = os.path.join(self.hyperparameters['save_path'], save_name + '.pkl')
        joblib.dump(results, save_path,compress=6)
        if self.config.visualise_overall_agent_results:
            plt.figure()
            y = np.array(results['reward'])
            plt.plot(np.arange(len(y)),y)
            plt.xlabel('Step')
            plt.ylabel('Reward')
            plt.grid()
            plt.title('Evaluation Rewards')
            save_path = os.path.join(self.hyperparameters['save_path'], save_name + '_reward.png')
    ...
    def locally_save_policy(self):
        """Takes a step in the game. This method must be overriden by any agent"""
        raise ValueError("locally_save_policy needs to be implemented by the agent")
    ...
    def load_model(self, model_path=None):
        """Takes a step in the game. This method must be overriden by any agent"""
        raise ValueError("load_model needs to be implemented by the agent")
