...
class Trainer(object):
    ...
    def __init__(self, config, agents):
        self.config = config
        self.agents = agents
        self.trained_agents =[[None]*config.runs_per_agent]*len(agents)
    ...
    def run_games_for_agents(self):
        ...
        if self.config.file_to_save_results_graph and self.config.visualise_overall_agent_results:
    ...
    def run_games_for_agent(self, agent_number, agent_class):
        ...
        for run in range(self.config.runs_per_agent):
            ...
            if self.config.visualise_individual_results:
                ...
                plt.show()
            agent_round += 1
            self.trained_agents[agent_number-1][run]=agent
