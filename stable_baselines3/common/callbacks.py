...
class EvalCallback(EventCallback):
    ...
    def __init__(
        ...
        self.step_report_reward = -np.inf
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, total_reward_list = evaluate_policy(
                self.model,
                self.eval_env,
                ...
                return_reward_list=True,
            )
            reward_list = total_reward_list[0]
            if self.log_path is not None:
                self.evaluations_timesteps.append(self.num_timesteps)
                self.evaluations_results.append(episode_rewards)
                self.evaluations_length.append(episode_lengths)
                self.evaluation_rewards.append(reward_list)
            ...
            self.reward_per_step = mean_reward/mean_ep_length
            step_reward_lasts = [np.mean(r_list[50:]) for r_list in reward_list]
            step_reward_bests = [np.max(r_list) for r_list in reward_list]
            self.step_reward_last = float(np.mean(step_reward_lasts))
            self.step_reward_best = float(np.mean(step_reward_bests))
            self.step_report_reward = (self.step_reward_last + self.step_reward_best)/2

            if self.verbose > 0:
                print(f"Eval num_timesteps={self.num_timesteps}, " f"episode_reward={mean_reward:.2f} +/- {std_reward:.2f}")
                print(f"Episode length: {mean_ep_length:.2f} +/- {std_ep_length:.2f}")
                print(f"rewards_per_step: {self.reward_per_step:.2f}", f" last_step_reward: {self.step_reward_last:.2f}", f" best_step_reward: {self.step_reward_best:.2f}")
            # Add to current Logger
            self.logger.record("eval/mean_reward", float(mean_reward))
            self.logger.record("eval/mean_ep_length", mean_ep_length)
            self.logger.record("eval/last_step_reward", self.step_reward_last)
            self.logger.record("eval/best_step_reward", self.step_reward_best)
